...
func TestAIConversationMessage(t *testing.T) {
	tests:=[]struct{name,nick string; ev bot.Event; want string; ok bool}{
	...
	for _,tt:=range tests{t.Run(tt.name,func(t *testing.T){got,ok:=aiConversationMessage(tt.nick,tt.ev);if got!=tt.want||ok!=tt.ok{t.Fatalf("got=(%q,%v)",got,ok)}})}
}
