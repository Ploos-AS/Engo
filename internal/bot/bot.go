...
func (b *Bot) Handle(m irc.Message) error {
	...
	if tagged, ok := m.Tags["account"]; ok {
		...
		if !accountVerified {
			...
			b.accounts[nickKey] = accountIdentity{account: account, userhost: currentUserhost}
		}
	}
	switch m.Command {
	case "ACCOUNT":
		...
		if len(m.Params) > 0 && m.Params[0] != "*" && currentUserhost != "" {
			account = m.Params[0]
	...
	case "NICK":
		...
		if newNick != "" && account != "" && accountVerified && currentUserhost != "" {
			delete(b.accounts, nickKey)
			b.accounts[ircNickKey(newNick, b.caseMapping)] = accountIdentity{account: account, userhost: currentUserhost}
		} else {
			delete(b.accounts, nickKey)
		}
	case "QUIT":
...
func eventFromMessage(m irc.Message) Event {
	...
	case "JOIN":
		...
		if len(m.Params) >= 2 {
			ev.Account = m.Params[1]
			if ev.Account == "*" {
				ev.Account = ""
			}
		}
		if len(m.Params) >= 3 {
			ev.RealName = m.Params[2]
		} else if len(m.Params) >= 2 {
			ev.RealName = m.Trailing
		}
	case "PART":
		ev.Name = "part"
	case "NOTICE":
	...
	case "ACCOUNT":
		ev.Name = "account"
		if len(m.Params) > 0 {
			ev.Account = m.Params[0]
			if ev.Account == "*" {
				ev.Account = ""
			}
		} else {
			ev.Account = m.Trailing
			if ev.Account == "*" {
				ev.Account = ""
			}
		}
...
func messageUserhost(m irc.Message) string {
	if i := strings.IndexByte(m.Prefix, '!'); i >= 0 && i+1 < len(m.Prefix) {
		return m.Prefix[i+1:]
	}
	return ""
}
