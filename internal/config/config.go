...
func validateCommandPermissions(raw string)error{raw=strings.TrimSpace(raw);if raw==""{return nil};seen:=make(map[string]bool);for _,entry:=range strings.Split(raw,","){parts:=strings.SplitN(strings.TrimSpace(entry),":",2);if len(parts)!=2||strings.TrimSpace(parts[0])==""||strings.TrimSpace(parts[1])==""||strings.Contains(parts[1],"+"){return fmt.Errorf("invalid ENGO_COMMAND_PERMISSIONS entry %q",entry)};command:=strings.ToLower(strings.TrimSpace(parts[0]));if err:=validateCommandName(command);err!=nil{return err};if seen[command]{return fmt.Errorf("duplicate command in ENGO_COMMAND_PERMISSIONS: %q",command)};seen[command]=true;if err:=validatePermissionName(strings.ToLower(strings.TrimSpace(parts[1])));err!=nil{return err}};return nil}
func validatePermissionName(name string)error{name=strings.TrimSpace(name);if name==""||len(name)>64||strings.Contains(name,".."){return fmt.Errorf("invalid permission name %q",name)};for i,r:=range name{if (r>='a'&&r<='z')||(r>='0'&&r<='9')||r=='-'||r=='_'||(r=='.'&&i>0&&i<len(name)-1){continue};return fmt.Errorf("invalid permission name %q",name)};return nil}
