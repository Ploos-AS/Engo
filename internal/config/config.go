...
func validatePermissionName(name string)error{name=strings.TrimSpace(name);if name==""||len(name)>64{return fmt.Errorf("invalid permission name %q",name)};for i,r:=range name{if (r>='a'&&r<='z')||(r>='0'&&r<='9')||r=='-'||r=='_'||(r=='.'&&i>0&&i<len(name)-1){continue};return fmt.Errorf("invalid permission name %q",name)};return nil}
