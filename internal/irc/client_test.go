...
func TestMultilineCapabilityCollection(t *testing.T) {
	...
	if !capLSContinues(first) {
		t.Fatal("expected CAP LS continuation")
	}
	...
	if !containsCapability(caps, "sasl") {
		t.Fatal("expected SASL from final CAP LS line")
	}
	if !containsCapability(caps, "multi-prefix") {
		t.Fatal("expected capability from continuation line")
	}
}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(chunks) != tc.want {
				t.Fatalf("got %d chunks, want %d", len(chunks), tc.want)
			}
			for i, ch := range chunks {
				if ch != "+" && len(ch) > 400 {
					t.Fatalf("chunk %d too long: %d", i, len(ch))
				}
			}
			if (chunks[len(chunks)-1] == "+") != tc.terminal {
				t.Fatalf("terminal marker mismatch: %#v", chunks)
			}
		})
	}
}
...
func TestCapabilityNamesFromNAK(t *testing.T) {
	...
	if !containsCapability(caps, "sasl") || !containsCapability(caps, "echo-message") {
		t.Fatalf("unexpected NAK capabilities: %#v", caps)
	}
}
...
func TestRegistrationTimeout(t *testing.T) {
	...
	err := c.Run()
	if err == nil || !strings.Contains(err.Error(), "registration timed out") {
		t.Fatalf("expected registration timeout, got %v", err)
	}
}
...
func TestRequestedCapabilitiesIncludeSASLOnce(t *testing.T) {
	...
	if count != 1 {
		t.Fatalf("SASL capability count = %d, want 1", count)
	}
}
...
func TestNonSASLCapabilityACKEndsNegotiation(t *testing.T) {
	...
	if !strings.Contains(string(buf[:n]), "CAP REQ :account-tag") {
		t.Fatalf("missing CAP REQ: %q", buf[:n])
	}
	...
	if !strings.Contains(string(buf[:n]), "CAP END") {
		t.Fatalf("non-SASL ACK did not end negotiation: %q", buf[:n])
	}
	_ = clientConn.Close()
	<-done
}
...
func TestPartialCapabilityACKWaitsForAllRequested(t *testing.T) {
	...
	if !strings.Contains(string(buf[:n]), "CAP REQ :account-tag server-time") {
		t.Fatalf("missing CAP REQ: %q", buf[:n])
	}
	if _, err := serverConn.Write([]byte(":irc.example CAP engo ACK :account-tag\r\n")); err != nil {
		t.Fatal(err)
	...
	if n, err = serverConn.Read(buf); err == nil && strings.Contains(string(buf[:n]), "CAP END") {
		t.Fatalf("CAP END sent before all capabilities ACKed")
	}
	...
	if !strings.Contains(string(buf[:n]), "CAP END") {
		t.Fatalf("CAP END missing after all ACKs: %q", buf[:n])
	}
	_ = clientConn.Close()
	<-done
}
...
func TestCapabilityNamesNormalizeModifiers(t *testing.T) {
	...
	for _, want := range []string{"account-tag", "server-time", "sasl"} {
		if !containsCapability(caps, want) {
			t.Fatalf("missing normalized capability %q in %#v", want, caps)
		}
	}
}
...
func TestNegativeCapabilityACKFailsFast(t *testing.T) {
	...
	select {
	case err := <-done:
		if err == nil || !strings.Contains(err.Error(), "disabled requested IRC capability") {
...
func TestCapabilityNAKReportsOnlyRequestedCapabilities(t *testing.T) {
	...
	case err := <-done:
		if err == nil {
			t.Fatal("expected requested capability rejection")
		}
		if !strings.Contains(err.Error(), "server-time") {
			t.Fatalf("missing requested capability in error: %v", err)
		}
		if strings.Contains(err.Error(), "unrelated-cap") {
			t.Fatalf("unrequested capability leaked into error: %v", err)
		}
	case <-time.After(100 * time.Millisecond):
		t.Fatal("CAP NAK did not fail fast")
	}
}
...
func TestUnrequestedCapabilityNAKIsIgnored(t *testing.T) {
	...
	if !strings.Contains(string(buf[:n]), "CAP END") {
		t.Fatalf("negotiation did not continue after unrelated NAK: %q", buf[:n])
	}
	_ = clientConn.Close()
	<-done
}
...
func TestCapabilityDELOfRequestedCapabilityFails(t *testing.T) {
	...
	select {
	case err := <-done:
		if err == nil || !strings.Contains(err.Error(), "removed requested IRC capability") {
			t.Fatalf("unexpected DEL result: %v", err)
		}
	case <-time.After(100 * time.Millisecond):
		t.Fatal("CAP DEL did not fail fast")
	}
}
...
func TestCapabilityNEWDoesNotEnableCapability(t *testing.T) {
	...
	if !strings.Contains(string(buf[:n]), "CAP END") {
		t.Fatalf("expected initial CAP END: %q", buf[:n])
	}
	...
	_ = serverConn.SetReadDeadline(time.Now().Add(30 * time.Millisecond))
	n, err = serverConn.Read(buf)
	if err == nil && strings.Contains(string(buf[:n]), "CAP REQ") {
		t.Fatalf("CAP NEW silently enabled capability")
	}
	_ = clientConn.Close()
	<-done
}
...
func TestActionUsesCTCPFramingAndSanitizesText(t *testing.T) {
	...
	if got != want {
		t.Fatalf("Action frame=%q want %q", got, want)
	}
	if strings.Contains(got, "\r\nPRIVMSG #other") {
		t.Fatal("Action allowed IRC line injection")
	}
}
...
func TestCAPDELSASLAfterAuthenticationDoesNotDisconnect(t *testing.T) {
	...
	write(":srv CAP engo LS :sasl")
	if line, err := read.ReadString('\n'); err != nil || !strings.Contains(line, "CAP REQ :sasl") {
		t.Fatalf("CAP REQ=%q err=%v", line, err)
	}
	write(":srv CAP engo ACK :sasl")
	if line, err := read.ReadString('\n'); err != nil || !strings.Contains(line, "AUTHENTICATE PLAIN") {
		t.Fatalf("AUTHENTICATE=%q err=%v", line, err)
	}
	write("AUTHENTICATE +")
	if _, err := read.ReadString('\n'); err != nil {
		t.Fatal(err)
	}
	write(":srv 903 engo :SASL authentication successful")
	if line, err := read.ReadString('\n'); err != nil || !strings.Contains(line, "CAP END") {
		t.Fatalf("CAP END=%q err=%v", line, err)
	}
	write(":srv 001 engo :welcome")
	write(":srv CAP engo DEL :sasl")
	_ = serverConn.Close()
	if err := <-done; err != io.EOF {
		t.Fatalf("CAP DEL sasl after auth returned %v, want EOF", err)
	}
}
...
func TestCAPDELRequiredRuntimeCapabilityDisconnects(t *testing.T) {
	for _, capability := range []string{"account-tag", "account-notify", "extended-join", "server-time"} {
		t.Run(capability, func(t *testing.T) {
			...
			read := bufio.NewReader(serverConn)
			write(":srv CAP engo LS :" + capability)
			if line, err := read.ReadString('\n'); err != nil || !strings.Contains(line, "CAP REQ :"+capability) {
				t.Fatalf("CAP REQ=%q err=%v", line, err)
			}
			write(":srv CAP engo ACK :" + capability)
			if line, err := read.ReadString('\n'); err != nil || !strings.Contains(line, "CAP END") {
				t.Fatalf("CAP END=%q err=%v", line, err)
			}
			write(":srv 001 engo :welcome")
			...
			err := <-done
			if err == nil || !strings.Contains(err.Error(), "server removed requested IRC capability") {
				t.Fatalf("CAP DEL %s returned %v", capability, err)
			}
		})
	}
}
...
func TestRunNormalizesConfiguredCapabilities(t *testing.T) {
	...
	if _, err := fmt.Fprintln(serverConn, ":server CAP * LS :account-tag"); err != nil {
		t.Fatal(err)
	}
	...
	if strings.TrimSpace(line) != "CAP REQ :account-tag" {
		t.Fatalf("unexpected CAP request %q", line)
	}
	if _, err := fmt.Fprintln(serverConn, ":server CAP * ACK :account-tag"); err != nil {
		t.Fatal(err)
	}
	line, err = r.ReadString('\n')
	if err != nil {
	...
	if strings.TrimSpace(line) != "CAP END" {
		t.Fatalf("unexpected CAP completion %q", line)
	}
	serverConn.Close()
...
func TestRunRejectsUnexpectedSASLSuccess(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() { errCh <- c.Run() }()
	if _, err := fmt.Fprintln(serverConn, ":server 903 nick :SASL authentication successful"); err != nil {
		t.Fatal(err)
	}
	select {
	case err := <-errCh:
		if err == nil || !strings.Contains(err.Error(), "unexpected SASL success numeric 903") {
			t.Fatalf("Run() error=%v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("Run() did not reject unexpected 903")
	}
}
...
func TestRunRejectsUnexpectedSASLFailureNumerics(t *testing.T) {
	for _, numeric := range []string{"904", "905", "906", "907"} {
		t.Run(numeric, func(t *testing.T) {
			...
			select {
			case err := <-errCh:
				if err == nil || !strings.Contains(err.Error(), "unexpected SASL failure numeric "+numeric) {
					t.Fatalf("Run() error=%v", err)
				}
			case <-time.After(time.Second):
				t.Fatalf("Run() did not reject unexpected %s", numeric)
			}
		})
	}
}
...
func TestRunRejectsCAPACKBeforeRequest(t *testing.T) {
	...
	select {
	case err := <-errCh:
		if err == nil || !strings.Contains(err.Error(), "unexpected CAP ACK before CAP REQ") {
			t.Fatalf("Run() error=%v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("Run() did not reject CAP ACK before CAP REQ")
	}
}

func TestRunRejectsUnrequestedCapabilityInACK(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	defer serverConn.Close()
	c := &Client{conn: clientConn, cfg: Config{Capabilities: []string{"account-tag"}}, registrationTimeout: time.Second}
	errCh := make(chan error, 1)
	go func() { errCh <- c.Run() }()
	r := bufio.NewReader(serverConn)
	if _, err := fmt.Fprintln(serverConn, ":server CAP * LS :account-tag server-time"); err != nil {
		t.Fatal(err)
	}
	line, err := r.ReadString('\n')
	if err != nil {
	...
	if _, err := fmt.Fprintln(serverConn, ":server CAP * ACK :account-tag server-time"); err != nil {
		t.Fatal(err)
	}
	select {
	case err := <-errCh:
		if err == nil || !strings.Contains(err.Error(), "server ACKed unrequested IRC capability \"server-time\"") {
			t.Fatalf("Run() error=%v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("Run() did not reject unrequested capability in ACK")
	}
}
...
func TestRunRejectsCAPNAKBeforeRequest(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() { errCh <- c.Run() }()
	if _, err := fmt.Fprintln(serverConn, ":server CAP * NAK :account-tag"); err != nil {
		t.Fatal(err)
	}
	select {
	case err := <-errCh:
		if err == nil || !strings.Contains(err.Error(), "unexpected CAP NAK before CAP REQ") {
			t.Fatalf("Run() error=%v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("Run() did not reject CAP NAK before CAP REQ")
	}
}
...
func TestRunRejectsCAPDELBeforeRequest(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() { errCh <- c.Run() }()
	if _, err := fmt.Fprintln(serverConn, ":server CAP * DEL :account-tag"); err != nil {
		t.Fatal(err)
	}
	select {
	case err := <-errCh:
		if err == nil || !strings.Contains(err.Error(), "unexpected CAP DEL before CAP REQ") {
			t.Fatalf("Run() error=%v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("Run() did not reject CAP DEL before CAP REQ")
	}
}

func TestRunRejectsCAPRepliesAfterEnd(t *testing.T) {
	for _, reply := range []string{"ACK", "NAK"} {
		t.Run(reply, func(t *testing.T) {
			...
			select {
			case err := <-errCh:
				if err == nil || !strings.Contains(err.Error(), "unexpected CAP "+reply+" after CAP END") {
					t.Fatalf("Run() error=%v", err)
				}
			case <-time.After(time.Second):
				t.Fatalf("Run() did not reject CAP %s after CAP END", reply)
			}
		})
	}
}
...
func TestRunRejectsRegistrationBeforeCAPComplete(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() { errCh <- c.Run() }()
	if _, err := fmt.Fprintln(serverConn, ":server 001 nick :Welcome"); err != nil {
		t.Fatal(err)
	}
	select {
	case err := <-errCh:
		if err == nil || !strings.Contains(err.Error(), "registered before capability negotiation completed") {
			t.Fatalf("Run() error=%v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("Run() accepted registration before CAP completed")
	}
}
...
func TestRunRejectsRegistrationBeforeSASLComplete(t *testing.T) {
	...
	go func() { errCh <- c.Run() }()
	r := bufio.NewReader(serverConn)
	if _, err := fmt.Fprintln(serverConn, ":server CAP * LS :sasl"); err != nil {
		t.Fatal(err)
	}
	if _, err := r.ReadString('\n'); err != nil {
		t.Fatal(err)
	}
	if _, err := fmt.Fprintln(serverConn, ":server CAP * ACK :sasl"); err != nil {
		t.Fatal(err)
	}
	line, err := r.ReadString('\n')
	if err != nil {
	...
	if strings.TrimSpace(line) != "AUTHENTICATE PLAIN" {
		t.Fatalf("unexpected SASL start %q", line)
	}
	if _, err := fmt.Fprintln(serverConn, ":server 001 nick :Welcome"); err != nil {
		t.Fatal(err)
	}
	...
	case <-time.After(time.Second):
		t.Fatal("Run() accepted registration before SASL completed")
	}
}
...
func TestRunAcceptsRegistrationAfterSASLComplete(t *testing.T) {
	...
	if strings.TrimSpace(line) != "CAP REQ :sasl" {
		t.Fatalf("unexpected CAP request %q", line)
	}
	if _, err := fmt.Fprintln(serverConn, ":server CAP * ACK :sasl"); err != nil {
		t.Fatal(err)
	...
	if _, err := fmt.Fprintln(serverConn, "AUTHENTICATE +"); err != nil {
		t.Fatal(err)
	}
	line, err = r.ReadString('\n')
	if err != nil {
	...
	if !strings.HasPrefix(strings.TrimSpace(line), "AUTHENTICATE ") {
		t.Fatalf("unexpected SASL payload %q", line)
	}
	if _, err := fmt.Fprintln(serverConn, ":server 903 nick :SASL authentication successful"); err != nil {
		t.Fatal(err)
	}
...
func TestRunRejectsLateCAPLS(t *testing.T) {
	...
	select {
	case err := <-errCh:
		if err == nil || !strings.Contains(err.Error(), "unexpected CAP LS after capability negotiation started") {
			t.Fatalf("Run() error=%v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("Run() did not reject late CAP LS")
	}
}

func TestRunNormalizesServerCapabilityCase(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	defer serverConn.Close()
	c := &Client{conn: clientConn, cfg: Config{Capabilities: []string{"account-tag"}}, registrationTimeout: time.Second}
	errCh := make(chan error, 1)
...
func TestRunRejectsCAPNEWBeforeCAPEND(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	defer serverConn.Close()
	c := &Client{conn: clientConn, cfg: Config{Capabilities: []string{"account-tag"}}, registrationTimeout: time.Second}
	errCh := make(chan error, 1)
	go func() { errCh <- c.Run() }()
	if _, err := fmt.Fprintln(serverConn, ":server CAP * NEW :server-time"); err != nil {
		t.Fatal(err)
	}
	select {
	case err := <-errCh:
		if err == nil || !strings.Contains(err.Error(), "unexpected CAP NEW before CAP END") {
			t.Fatalf("Run() error=%v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("Run() did not reject early CAP NEW")
	}
}

func TestRunIgnoresCAPNEWAfterCAPEND(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	defer serverConn.Close()
	c := &Client{conn: clientConn, cfg: Config{Capabilities: []string{"account-tag"}}, registrationTimeout: time.Second}
	errCh := make(chan error, 1)
	...
	if line, err := r.ReadString('\n'); err != nil || strings.TrimSpace(line) != "CAP REQ :account-tag" {
		t.Fatalf("CAP REQ line=%q err=%v", line, err)
	}
	if _, err := fmt.Fprintln(serverConn, ":server CAP * ACK :account-tag"); err != nil {
		t.Fatal(err)
	}
	if line, err := r.ReadString('\n'); err != nil || strings.TrimSpace(line) != "CAP END" {
		t.Fatalf("CAP END line=%q err=%v", line, err)
	}
	if _, err := fmt.Fprintln(serverConn, ":server 001 nick :Welcome"); err != nil {
		t.Fatal(err)
	}
	if _, err := fmt.Fprintln(serverConn, ":server CAP nick NEW :server-time"); err != nil {
		t.Fatal(err)
	}
	if _, err := fmt.Fprintln(serverConn, "PING :probe"); err != nil {
		t.Fatal(err)
	}
	if line, err := r.ReadString('\n'); err != nil || strings.TrimSpace(line) != "PONG :probe" {
...
func TestRunIgnoresCAPDELForUnrequestedCapabilityAfterRegistration(t *testing.T) {
	...
	if _, err := fmt.Fprintln(serverConn, ":server CAP nick DEL :server-time"); err != nil {
		t.Fatal(err)
	}
	if _, err := fmt.Fprintln(serverConn, "PING :probe"); err != nil {
		t.Fatal(err)
...
func TestRunFailsClosedWhenRequestedCapabilityRemovedAfterRegistration(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	defer serverConn.Close()
	c := &Client{conn: clientConn, cfg: Config{Capabilities: []string{"account-tag"}}, registrationTimeout: time.Second}
	errCh := make(chan error, 1)
	...
	if _, err := fmt.Fprintln(serverConn, ":server CAP nick DEL :account-tag"); err != nil {
		t.Fatal(err)
	}
	select {
	case err := <-errCh:
		if err == nil || !strings.Contains(err.Error(), "server removed requested IRC capability \"account-tag\"") {
			t.Fatalf("Run() error=%v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("Run() did not fail after requested capability removal")
	}
}
