...
func TestRunRejectsCAPRepliesAfterEnd(t *testing.T){
 for _,reply:=range []string{"ACK","NAK"}{t.Run(reply,func(t *testing.T){
  clientConn,serverConn:=net.Pipe();defer serverConn.Close()
  c:=&Client{conn:clientConn,cfg:Config{Capabilities:[]string{"account-tag"}},registrationTimeout:time.Second}
...
func TestRunRejectsCAPNEWBeforeCAPEND(t *testing.T){
 ...
 c:=&Client{conn:clientConn,cfg:Config{Capabilities:[]string{"account-tag"}},registrationTimeout:time.Second}
 errCh:=make(chan error,1);go func(){errCh<-c.Run()}()
 if _,err:=fmt.Fprintln(serverConn,":server CAP * NEW :server-time");err!=nil{t.Fatal(err)}
 select{case err:=<-errCh:if err==nil||!strings.Contains(err.Error(),"unexpected CAP NEW before CAP END"){t.Fatalf("Run() error=%v",err)};case <-time.After(time.Second):t.Fatal("Run() did not reject early CAP NEW")}
}
...
func TestRunIgnoresCAPNEWAfterCAPEND(t *testing.T){
 ...
 if _,err:=fmt.Fprintln(serverConn,":server 001 nick :Welcome");err!=nil{t.Fatal(err)}
 if _,err:=fmt.Fprintln(serverConn,":server CAP nick NEW :server-time");err!=nil{t.Fatal(err)}
