...
func TestParseMessageTags(t *testing.T) {
	...
	if m.Tags["time"] != "2026-09-25T09:30:00.000Z" { t.Fatalf("unexpected time tag: %q", m.Tags["time"]) }
	if m.Tags["account"] != "alice user" { t.Fatalf("unexpected account tag: %q", m.Tags["account"]) }
	if m.Nick != "alice" || m.Command != "PRIVMSG" || m.Trailing != "hello" { t.Fatalf("unexpected message: %#v", m) }
}
