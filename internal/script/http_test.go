...
func TestHTTPDisabledByDefault(t *testing.T) {
	...
	if h.Enabled() {
		t.Fatal("HTTP must be disabled without allowlist")
	}
}
...
func TestBlockedIP(t *testing.T) {
	...
	if blockedIP(net.ParseIP("8.8.8.8")) {
		t.Fatal("public address unexpectedly blocked")
	}
}
...
func TestHTTPRejectsNon443Port(t *testing.T) {
	...
	if err := h.validateURL(u); err == nil {
		t.Fatal("expected non-443 port rejection")
	}
}
func TestHTTPTransportIgnoresEnvironmentProxy(t *testing.T) {
	...
	if tr.Proxy != nil {
		t.Fatal("HTTP transport must not use environment proxy")
	}
}
...
func TestHTTPRejectsIPv4MappedPrivateAddress(t *testing.T) {
	...
	if ip == nil {
		t.Fatal("failed to parse mapped IPv4 address")
	}
	if !blockedIP(ip) {
		t.Fatal("IPv4-mapped loopback address should be blocked")
	}
}
...
func TestHTTPRejectsAllowedHostResolvingPrivate(t *testing.T) {
	...
	if err := h.validateURL(u); err == nil {
		t.Fatal("expected private resolved address rejection")
	}
}
...
func TestHTTPRedirectRejectsUnlistedHost(t *testing.T) {
	...
	if err := h.client.CheckRedirect(req, nil); err == nil {
...
func TestHTTPRedirectLimit(t *testing.T) {
	...
	if err := h.client.CheckRedirect(req, via); err == nil {
...
func TestHTTPResponseBodyLimit(t *testing.T) {
	...
	if _, err := h.Get("https://allowed.example/"); err == nil {
...
func TestHTTPRejectsHostWithNoAddresses(t *testing.T) {
	...
	u, _ := url.Parse("https://allowed.example/")
	if err := h.validateURL(u); err == nil {
		t.Fatal("expected empty DNS result rejection")
	}
}
