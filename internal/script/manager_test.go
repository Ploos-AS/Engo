...
if bot("active","ok"){bot("say",event["target"],"old")}`)
	...
	if err := os.WriteFile(filepath.Join(dir, "broken.tengo"), []byte("{{ invalid"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := m.ReloadAll(); err == nil {
	...
	if _, text := s.snapshot(); text != "old" {
		t.Fatalf("old registry not preserved: %q", text)
	}
}
...
if bot("active","two"){bot("say",event["target"],"two")}`)
	...
	if err := m.Disable("two.tengo"); err != nil {
		t.Fatal(err)
	}
	s.reset()
	_ = b.Handle(irc.ParseMessage(":a!u@h PRIVMSG #x :!two"))
	if _, text := s.snapshot(); text != "" {
		t.Fatalf("disabled script still handled command: %q", text)
	}
	if got := m.Disabled(); len(got) != 1 || got[0] != "two.tengo" {
		t.Fatalf("disabled=%v", got)
	}
	if err := m.Enable("two.tengo"); err != nil {
		t.Fatal(err)
	}
	_ = b.Handle(irc.ParseMessage(":a!u@h PRIVMSG #x :!two"))
	if _, text := s.snapshot(); text != "two" {
		t.Fatalf("enabled script did not return: %q", text)
	}
}

func TestManagerRejectsPathTraversal(t *testing.T) {
	m := NewManager(t.TempDir(), bot.New(&captureSender{}))
	if err := m.Disable("../outside.tengo"); err == nil {
		t.Fatal("expected invalid script name")
	}
}
...
if bot("active","one"){bot("say",event["target"],"old")}`)
	...
	if err := os.WriteFile(one, []byte("{{ invalid"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := m.Disable("two.tengo"); err == nil {
		t.Fatal("expected disable reload failure")
	}
	if got := m.Disabled(); len(got) != 0 {
		t.Fatalf("disabled state not rolled back: %v", got)
	}
	s.reset()
	_ = b.Handle(irc.ParseMessage(":a!u@h PRIVMSG #x :!one"))
	if _, text := s.snapshot(); text != "old" {
		t.Fatalf("old registry not preserved after failed disable: %q", text)
	}
}
...
if bot("active","one"){bot("say",event["target"],"old")}`)
	...
	if err := m.Enable("two.tengo"); err == nil {
		t.Fatal("expected enable reload failure")
	}
	if got := m.Disabled(); len(got) != 1 || got[0] != "two.tengo" {
		t.Fatalf("disabled state not rolled back: %v", got)
	}
...
if bot("active","fired"){bot("say","#x","timer-fired")}`)
	...
	for time.Now().Before(deadline) {
		if _, text := s.snapshot(); text == "timer-fired" {
			return
		}
...
if bot("active","new"){bot("say",event["target"],"new-generation")}`)
	...
	if _, text := s.snapshot(); text != "new-generation" {
		t.Fatalf("new registry not active: %q", text)
	}
	s.reset()
	time.Sleep(1200 * time.Millisecond)
	if _, text := s.snapshot(); text != "" {
		t.Fatalf("old generation timer fired after successful reload: %q", text)
	}
}
...
if bot("active","set"){kv_set("value","persisted")}`)
	s := &captureSender{}
	b := bot.New(s)
	m := NewManagerWithState(dir, b, 100000, stateDir)
	if err := m.ReloadAll(); err != nil {
		t.Fatal(err)
	}
...
if bot("active","get"){
	...
	if _, text := s.snapshot(); text != "persisted" {
		t.Fatalf("store state did not survive reload: %q", text)
	}
}
...
if bot("active","get"){
	...
	_ = b.Handle(irc.ParseMessage(":a!u@h PRIVMSG #x :!set"))
	if err := m.Disable("state.tengo"); err != nil {
		t.Fatal(err)
	}
	s.reset()
	_ = b.Handle(irc.ParseMessage(":a!u@h PRIVMSG #x :!get"))
	if s.text != "" {
		t.Fatalf("disabled script still handled command: %q", s.text)
	}
	if err := m.Enable("state.tengo"); err != nil {
		t.Fatal(err)
	}
	_ = b.Handle(irc.ParseMessage(":a!u@h PRIVMSG #x :!get"))
	if _, text := s.snapshot(); text != "persisted" {
		t.Fatalf("store state did not survive disable/enable: %q", text)
	}
}
...
func TestManagerCopiesPermissionPolicyOnAssignment(t *testing.T) {
	b := bot.New(&captureSender{})
	m := NewManager(t.TempDir(), b)
	permissions := map[string][]string{"alice": {"admin"}}
	commands := map[string]string{"reload": "admin"}
	m.SetPermissions(permissions)
	m.SetCommandPermissions(commands)
	permissions["alice"][0] = "mutated"
	permissions["bob"] = []string{"admin"}
	commands["reload"] = "mutated"
	m.mu.RLock()
	defer m.mu.RUnlock()
	if got := m.permissions["alice"][0]; got != "admin" {
		t.Fatalf("permission policy aliased caller data: %q", got)
	}
	if _, ok := m.permissions["bob"]; ok {
		t.Fatal("permission map aliased caller data")
	}
	if got := m.commandPermissions["reload"]; got != "admin" {
		t.Fatalf("command policy aliased caller data: %q", got)
	}
}
...
func TestManagerDisableRejectsDirectoryNamedTengo(t *testing.T) {
	dir := t.TempDir()
	if err := os.Mkdir(filepath.Join(dir, "nested.tengo"), 0o700); err != nil {
		t.Fatal(err)
	}
	m := NewManager(dir, bot.New(&captureSender{}))
	if err := m.Disable("nested.tengo"); err == nil {
...
if bot("active","one"){bot("say",event["target"],"new-one")}`)
	if err := os.WriteFile(two, []byte("{{ invalid"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := m.Reload("one.tengo"); err == nil {
		t.Fatal("expected atomic named reload failure")
	}
	s.reset()
	_ = b.Handle(irc.ParseMessage(":a!u@h PRIVMSG #x :!one"))
	if _, text := s.snapshot(); text != "old-one" {
		t.Fatalf("old registry not preserved: %q", text)
	}
}
