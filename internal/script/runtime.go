...
func (r *Runtime) eventCallFor(active string,currentEvent bot.Event)func(...tengo.Object)(tengo.Object,error){
	return func(args ...tengo.Object)(tengo.Object,error){
		...
		case "timer_every":
			...
			d,err:=parseTimerDuration(duration);if err!=nil{return nil,err}
			timerID:=id
			if err:=r.scheduler.Every(timerID,d,func(){if err:=r.runHandler(r.currentSource(),id,bot.Event{Name:"timer"});err!=nil{fmt.Fprintf(os.Stderr, "Engo timer %s: %v\n", id, err)}});err!=nil{return nil,err}
			return tengo.UndefinedValue,nil
		case "timer_cancel":
...
func (r *Runtime) allowed(ev bot.Event,perm string)bool{if !ev.AccountVerified{return false};account:=strings.ToLower(eventAccount(ev));return account!=""&&r.permissions[account][strings.ToLower(strings.TrimSpace(perm))]}
