...
func (s *captureSender) Say(target, text string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *captureSender) snapshot() (string, string) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
if bot("active","hello") { bot("say",event["target"],"Hello "+event["nick"]) }`)
	...
	if target != "#engo" || text != "Hello alice" {
		t.Fatalf("unexpected reply: target=%q text=%q", target, text)
	}
}
...
if bot("active","hello") { bot("say",event["target"],"v1") }`
	...
	_ = b.Handle(irc.ParseMessage(":alice!u@example PRIVMSG #engo :!hello"))
	if sender.text != "v1" {
		t.Fatalf("previous script was not preserved: %#v", sender)
	}
}
...
if bot("active","hello") { bot("say",event["target"],"v2") }`
	...
	_ = b.Handle(irc.ParseMessage(":alice!u@example PRIVMSG #engo :!hello"))
	if sender.text != "v2" {
		t.Fatalf("new script not active: %#v", sender)
	}
}
...
if bot("active","fetch") { bot("http_get","https://example.com/") }`)
	...
	if err := b.Handle(irc.ParseMessage(":alice!u@example PRIVMSG #engo :!fetch")); err != nil {
		t.Fatal(err)
	}
	if _, text := sender.snapshot(); text != "" {
		t.Fatalf("denied HTTP command produced output: %q", text)
	}
}
...
if bot("active","fetch") { bot("http_get","http://example.com/") }`)
	...
	if _, text := sender.snapshot(); text != "" {
		t.Fatalf("rejected HTTP request produced output: %q", text)
	}
}
...
func TestEventObjectExposesIRCv3Tags(t *testing.T) {
	...
	obj := eventObject(ev)
	if obj["time"] != "2026-09-25T09:30:00.000Z" {
		t.Fatalf("unexpected time: %#v", obj["time"])
	}
	if obj["account"] != "alice" {
		t.Fatalf("unexpected account: %#v", obj["account"])
	}
	tags, ok := obj["tags"].(map[string]interface{})
	if !ok {
		t.Fatalf("unexpected tags type: %T", obj["tags"])
	}
	if tags["time"] != "2026-09-25T09:30:00.000Z" || tags["account"] != "alice" {
...
func TestExtendedJoinIdentityInEvent(t *testing.T) {
	...
	if obj["realname"] != "Alice Example" {
		t.Fatalf("unexpected realname: %#v", obj["realname"])
	}
}
...
func TestAccountNotifyEventObject(t *testing.T) {
	for _, tc := range []struct{ line, want string }{
		...
		if obj["account"] != tc.want {
			t.Fatalf("%q: account=%#v, want %q", tc.line, obj["account"], tc.want)
		}
	}
}
...
if bot("active","who") { bot("say",event["target"],event["account"]) }`)
	...
	if err := b.Handle(irc.ParseMessage(":alice!u@example ACCOUNT alice-account")); err != nil {
		t.Fatal(err)
	}
	if err := b.Handle(irc.ParseMessage(":alice!u@example PRIVMSG #engo :!who")); err != nil {
		t.Fatal(err)
	}
	if _, text := sender.snapshot(); text != "alice-account" {
		t.Fatalf("persisted account=%q", text)
	}
	if err := b.Handle(irc.ParseMessage(":alice!u@example ACCOUNT *")); err != nil {
		t.Fatal(err)
	}
	sender.mu.Lock()
	...
	if _, text := sender.snapshot(); text != "" {
		t.Fatalf("account should be cleared after logout, got %q", text)
	}
}
...
if bot("active","who") { bot("say",event["target"],event["account"]) }`)
	...
	if err := b.Handle(irc.ParseMessage(":alice!u@example JOIN #engo alice-account :Alice Example")); err != nil {
		t.Fatal(err)
	}
	if err := b.Handle(irc.ParseMessage(":alice!u@example NICK :alice2")); err != nil {
		t.Fatal(err)
	}
	if err := b.Handle(irc.ParseMessage(":alice2!u@example PRIVMSG #engo :!who")); err != nil {
		t.Fatal(err)
	}
	if _, text := sender.snapshot(); text != "alice-account" {
		t.Fatalf("account after nick=%q", text)
	}
	if err := b.Handle(irc.ParseMessage(":alice2!u@example QUIT :bye")); err != nil {
		t.Fatal(err)
	}
	sender.mu.Lock()
	sender.text = "sentinel"
	...
	if _, text := sender.snapshot(); text != "" {
		t.Fatalf("account after quit=%q", text)
	}
}
...
if bot("active","admin") { if bot("allowed","admin") { bot("say",event["target"],"yes") } else { bot("say",event["target"],"no") } }`)
	...
	if err := b.Handle(irc.ParseMessage(":bob!u@example PRIVMSG #engo :!admin")); err != nil {
		t.Fatal(err)
	}
	if _, text := sender.snapshot(); text != "no" {
		t.Fatalf("unauthenticated/default permission result=%q", text)
	}
	if err := b.Handle(irc.ParseMessage(":alice!u@example ACCOUNT alice-account")); err != nil {
		t.Fatal(err)
	}
	if err := b.Handle(irc.ParseMessage(":alice!u@example PRIVMSG #engo :!admin")); err != nil {
		t.Fatal(err)
	}
	if _, text := sender.snapshot(); text != "yes" {
		t.Fatalf("granted permission result=%q", text)
	}
}
...
if bot("active","reload") { bot("say",event["target"],"ran") }`)
	...
	sender.mu.Unlock()
	if err := b.Handle(irc.ParseMessage(":bob!u@example PRIVMSG #engo :!reload")); err != nil {
		t.Fatal(err)
	}
	if _, text := sender.snapshot(); text != "untouched" {
		t.Fatalf("unauthenticated protected command ran: %q", text)
	}

	if err := b.Handle(irc.ParseMessage(":bob!u@example ACCOUNT bob-account")); err != nil {
		t.Fatal(err)
	}
	if err := b.Handle(irc.ParseMessage(":bob!u@example PRIVMSG #engo :!reload")); err != nil {
		t.Fatal(err)
	...
	if _, text := sender.snapshot(); text != "untouched" {
		t.Fatalf("unauthorized protected command ran: %q", text)
	}

	if err := b.Handle(irc.ParseMessage(":alice!u@example ACCOUNT alice-account")); err != nil {
		t.Fatal(err)
	}
	if err := b.Handle(irc.ParseMessage(":alice!u@example PRIVMSG #engo :!reload")); err != nil {
		t.Fatal(err)
	}
	if _, text := sender.snapshot(); text != "ran" {
		t.Fatalf("authorized protected command did not run: %q", text)
	}
}
...
func TestProtectedCommandRejectsUnverifiedAccountName(t *testing.T) {
	...
	if rt.allowed(ev, "admin") {
		t.Fatal("unverified account name granted permission")
	}
}
...
func TestAccountTagGrantsProtectedCommand(t *testing.T) {
	...
	if err := b.Handle(irc.ParseMessage("@account=alice-account :alice!u@example PRIVMSG #engo :!reload")); err != nil {
		t.Fatal(err)
	}
	if _, text := sender.snapshot(); text != "ran" {
		t.Fatalf("verified account-tag did not authorize protected command: %q", text)
	}
}

func TestEventObjectExposesAccountVerification(t *testing.T) {
	ev := bot.Event{Name: "message", Account: "alice-account", AccountVerified: true}
	obj := eventObject(ev)
	if obj["account_verified"] != true {
		t.Fatalf("account_verified=%#v", obj["account_verified"])
	}
}
...
func TestAccountNotifyCacheGrantsProtectedCommand(t *testing.T) {
	...
	if _, text := sender.snapshot(); text != "ran" {
		t.Fatalf("account-notify cache did not authorize protected command: %q", text)
	}
}
...
func TestAccountNotifyCacheDifferentUserhostDenied(t *testing.T) {
	...
	if err := b.Handle(irc.ParseMessage(":alice!old@example ACCOUNT alice-account")); err != nil {
		t.Fatal(err)
	}
	sender.mu.Lock()
	sender.text = "untouched"
	sender.mu.Unlock()
	if err := b.Handle(irc.ParseMessage(":alice!new@example PRIVMSG #engo :!reload")); err != nil {
		t.Fatal(err)
	}
	if _, text := sender.snapshot(); text != "untouched" {
		t.Fatalf("stale account-notify identity authorized command: %q", text)
	}
}
...
func TestExtendedJoinCacheGrantsProtectedCommand(t *testing.T) {
	...
	if _, text := sender.snapshot(); text != "ran" {
		t.Fatalf("extended-join identity did not authorize protected command: %q", text)
	}
}
...
func TestAccountLogoutRevokesProtectedCommand(t *testing.T) {
	...
	if _, text := sender.snapshot(); text != "untouched" {
		t.Fatalf("logout did not revoke protected command: %q", text)
	}
}
...
func TestQuitRevokesProtectedCommand(t *testing.T) {
	...
	if err := b.Handle(irc.ParseMessage(":alice!u@example QUIT :bye")); err != nil {
		t.Fatal(err)
	}
	sender.mu.Lock()
	sender.text = "untouched"
	...
	if _, text := sender.snapshot(); text != "untouched" {
		t.Fatalf("QUIT did not revoke protected command: %q", text)
	}
}
...
func TestNickChangeKeepsProtectedCommandForSameUserhost(t *testing.T) {
	...
	if err := b.Handle(irc.ParseMessage(":alice2!u@example PRIVMSG #engo :!reload")); err != nil {
		t.Fatal(err)
	}
	if _, text := sender.snapshot(); text != "ran" {
		t.Fatalf("same-userhost nick change lost authorization: %q", text)
	}
}
