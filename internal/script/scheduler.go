...
func NewScheduler() *Scheduler {
	return &Scheduler{timers: make(map[string]timerEntry)}
}
...
func (s *Scheduler) After(id string, d time.Duration, fn func()) error {
	...
	if len(s.timers) >= maxActiveTimers {
		s.mu.Unlock()
		return fmt.Errorf("timer limit reached")
	}
	token := &struct{}{}
	...
	s.timers[id] = timerEntry{stop: t.Stop, token: token}
	s.mu.Unlock()
	return nil
}
...
func (s *Scheduler) Every(id string, d time.Duration, fn func()) error {
	...
	s.timers[id] = timerEntry{stop: stop, token: token}
	s.mu.Unlock()
...
func parseTimerDuration(v string) (time.Duration, error) {
	...
	if err != nil || d < time.Second || d > 30*24*time.Hour {
		return 0, fmt.Errorf("timer duration must be between 1s and 720h")
	}
	return d, nil
}
