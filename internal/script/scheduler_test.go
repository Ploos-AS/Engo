...
func TestSchedulerCancel(t *testing.T) {
	...
	if !s.Cancel("x") {
		t.Fatal("expected timer cancellation")
	}
...
func TestParseTimerDurationRejectsTooSmall(t *testing.T) {
	if _, err := parseTimerDuration("0s"); err == nil {
...
func TestSchedulerEveryAndCancel(t *testing.T) {
	...
	for count.Load() < 2 {
		select {
		case <-deadline:
			t.Fatal("repeating timer did not fire twice")
		default:
	...
	atCancel := count.Load()
	time.Sleep(1100 * time.Millisecond)
	if count.Load() != atCancel {
		t.Fatalf("timer continued after cancel: %d -> %d", atCancel, count.Load())
	}
}
...
func TestSchedulerTimerLimit(t *testing.T) {
	...
	for i := 0; i < maxActiveTimers; i++ {
		if err := s.After(fmt.Sprintf("timer-%d", i), time.Hour, func() {}); err != nil {
			t.Fatalf("timer %d: %v", i, err)
		}
	}
	if err := s.After("overflow", time.Hour, func() {}); err == nil {
		t.Fatal("expected timer limit error")
	}
	s.CancelAll()
}
...
func TestSchedulerReplacingEveryStopsOld(t *testing.T) {
	...
	if err := s.Every("same", 20*time.Millisecond, func() { old.Add(1) }); err != nil {
		t.Fatal(err)
	}
	time.Sleep(30 * time.Millisecond)
	if err := s.Every("same", 20*time.Millisecond, func() { replacement.Add(1) }); err != nil {
		t.Fatal(err)
	}
	...
	for replacement.Load() == 0 {
		select {
		case <-deadline:
			t.Fatal("replacement repeating timer did not fire")
		default:
			time.Sleep(5 * time.Millisecond)
		}
	}
	time.Sleep(50 * time.Millisecond)
	if old.Load() != oldAtReplace {
		t.Fatalf("replaced repeating timer continued: %d -> %d", oldAtReplace, old.Load())
	}
	s.CancelAll()
}
...
func TestSchedulerCancelEveryPreventsFurtherCallbacks(t *testing.T) {
	s := NewScheduler()
	var count atomic.Int32
	if err := s.Every("repeat", 20*time.Millisecond, func() { count.Add(1) }); err != nil {
		t.Fatal(err)
	}
	deadline := time.After(500 * time.Millisecond)
	for count.Load() == 0 {
		select {
		case <-deadline:
			t.Fatal("repeating timer did not fire")
		default:
			time.Sleep(5 * time.Millisecond)
		}
	}
	if !s.Cancel("repeat") {
		t.Fatal("expected repeating timer cancellation")
	}
	atCancel := count.Load()
	time.Sleep(60 * time.Millisecond)
	if count.Load() != atCancel {
		t.Fatalf("callback ran after cancellation: %d -> %d", atCancel, count.Load())
	}
}
