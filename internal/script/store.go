...
func NewStore(root, namespace string) *Store {
	key := filepath.Join(root, namespace)
	v, _ := storeLocks.LoadOrStore(key, &sync.Mutex{})
	return &Store{root: root, namespace: namespace, mu: v.(*sync.Mutex)}
}
...
func (s *Store) Set(key, value string) error {
	...
	if len(value) > 65536 {
		return fmt.Errorf("KV value too large")
	}
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) write(m map[string]string) error {
	...
	tmp, err := os.CreateTemp(s.root, s.namespace+".*.tmp")
	if err != nil {
		return err
	}
	tmpName := tmp.Name()
...
func validKey(key string) error {
	if key == "" || len(key) > 128 || strings.ContainsAny(key, "\r\n") {
		return fmt.Errorf("invalid KV key")
	}
	return nil
}
