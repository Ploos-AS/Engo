...
func TestStoreCrossProcessHelper(t *testing.T){
	if os.Getenv("ENGO_STORE_HELPER")!="1"{return}
	root:=os.Getenv("ENGO_STORE_ROOT")
	key:=os.Getenv("ENGO_STORE_KEY")
	if err:=NewStore(root,"shared").Set(key,"value-"+key);err!=nil{t.Fatal(err)}
}
...
func TestStoreConcurrentProcesses(t *testing.T){
	...
	for i:=0;i<n;i++{
		...
		if err!=nil||!ok||got!="value-"+key{t.Fatalf("missing cross-process key %s: %q %v %v",key,got,ok,err)}
	}
}
